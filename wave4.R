...
recipe_obj_G7 <- recipe(cogscore ~ ., data = G7_wave4_data) %>%
...
recipe_obj_G7_rf <- recipe(cogscore ~ ., data = G7_wave4_data) %>%
  step_center(age, bmi) %>%
  step_scale(age, bmi)
...
library(caret)
set.seed(3456)
trainIndex_G7 <- createDataPartition(G7_wave4_data$cogscore, p = .8, 
                                     list = FALSE, 
                                     times = 1)


G7_wave4_Train <- G7_wave4_data[ trainIndex_G7,]
G7_wave4_Test  <- G7_wave4_data[-trainIndex_G7,]
...
explainer_regr_lm_G7 <- DALEX::explain(lm_model_G7, label = "lm", 
                                       data = G7_wave4_Test, y = G7_wave4_Test$cogscore, 
                                       verbose = FALSE)
...
spec_obs <- G7_wave4_data %>% filter(gender == 1  & age > 69 & age < 71 &
                                       isced == 3 &
                                       eurod == 1 &
                                       br010 == 2 & br015 == 2 ) %>% 
  select(!c('cogscore'))

G7_wave4_data %>% filter(gender == 1  & age > 69 & age < 71 &
                           isced == 3 &
                           eurod == 1 &
                           br010 == 2 & br015 == 2 )
spec_obs
