...
recipe_obj_nonG7 <- recipe(cogscore ~ ., data = nonG7_wave5_data) %>%
...
recipe_obj_nonG7_rf <- recipe(cogscore ~ ., data = nonG7_wave5_data) %>%
  step_center(age, bmi) %>%
  step_scale(age, bmi)
...
explainer_regr_lm_nonG7 <- DALEX::explain(lm_model_nonG7, label = "lm", 
                                          data = nonG7_wave5_Test, y = nonG7_wave5_Test$cogscore, 
                                          verbose = FALSE)
...
spec_obs <- nonG7_wave5_data %>% filter(gender == 1  & age > 69 & age < 71 &
                                          isced == 3 & eurod == 1 & 
                                          br010 == 1 & br015 == 4) %>% 
  select(!c('cogscore'))

nonG7_wave5_data %>% filter(gender == 1  & age > 69 & age < 71 &
                              isced == 3 & eurod == 1 & 
                              br010 == 1 & br015 == 4)
spec_obs
