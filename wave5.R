...
# Modeling part wave_5 
# G7 vs Non G7 countries
G7_wave5_data <- selected_data_wave_5 %>% filter(country_tag == 'G7') %>% select(gender, age, isced, eurod, bmi, 
                                                                                 br010, br015, smoking, chronic, cogscore)

nonG7_wave5_data <- selected_data_wave_5 %>% filter(country_tag == 'Non_G7') %>% select(gender, age, isced, eurod, bmi, 
                                                                                        br010, br015, smoking, chronic, cogscore)

library(recipes)
# Create the preprocessing recipe for G7
recipe_obj_G7 <- recipe(cogscore ~ ., data = G7_wave5_data) %>%
...
recipe_obj_G7_rf <- recipe(cogscore ~ ., data = G7_wave5_data) %>%
  step_center(age, bmi) %>%
  step_scale(age, bmi)
...
library(caret)
set.seed(3456)
trainIndex_G7 <- createDataPartition(G7_wave5_data$cogscore, p = .8, 
                                     list = FALSE, 
                                     times = 1)


G7_wave5_Train <- G7_wave5_data[ trainIndex_G7,]
G7_wave5_Test  <- G7_wave5_data[-trainIndex_G7,]

trainIndex_nonG7 <- createDataPartition(nonG7_wave5_data$cogscore, p = .8, 
                                        list = FALSE, 
                                        times = 1)

nonG7_wave5_Train <- nonG7_wave5_data[ trainIndex_nonG7,]
nonG7_wave5_Test  <- nonG7_wave5_data[-trainIndex_nonG7,]
...
explainer_regr_lm_G7 <- DALEX::explain(lm_model_G7, label = "lm", 
                                       data = G7_wave5_Test, y = G7_wave5_Test$cogscore, 
                                       verbose = FALSE)
...
spec_obs <- G7_wave5_data %>% filter(gender == 1  & age > 69 & age < 71 &
                                       isced == 3 & eurod == 1 & 
                                       br010 == 2 & br015 == 4) %>% 
  select(!c('cogscore'))

G7_wave5_data %>% filter(gender == 1  & age > 69 & age < 71 &
                           isced == 3 & eurod == 1 & 
                           br010 == 2 & br015 == 4)
spec_obs
